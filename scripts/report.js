...
function findingCard(f, idx) {
  ...
  <div class="${style.bg} ${style.text} border-l-4 ${style.border} p-5 rounded shadow-sm mb-3">
    ...
    <details class="text-xs">
      <summary class="cursor-pointer font-semibold text-green-800">Fix SQL (copy & run in Nhost SQL editor)</summary>
...
export function renderHtml(result) {
  const { nhost_endpoint, scanned_at, summary, findings, n_sources, n_tables, active_probe } = result;
  const project_name = nhost_endpoint;
  const project_ref = nhost_endpoint;
  const region = "Hasura/Nhost instance";
  const n_tables_scanned = n_tables;
  const n_functions_scanned = n_sources;
...
<head>
  <meta charset="UTF-8">
  <title>Nhost Security Report — ${escapeHtml(project_name)}</title>
...
<body class="bg-gray-50 text-gray-900 font-sans">
  <div class="max-w-5xl mx-auto p-6">
    ...
    <div class="bg-gradient-to-r from-emerald-700 to-teal-600 text-white p-8 rounded-lg shadow-lg mb-6">
      <h1 class="text-3xl font-bold mb-2">Nhost Security Report</h1>
    ...
    <div class="bg-white p-5 rounded-lg shadow mb-6">
      ...
      <div class="grid grid-cols-3 gap-4 text-center">
        ...
        <div><div class="text-2xl font-bold text-gray-700">${n_functions_scanned}</div><div class="text-xs text-gray-500">Sources scanned</div></div>
        <div><div class="text-2xl font-bold text-gray-700">5</div><div class="text-xs text-gray-500">Roles audited</div></div>
      </div>
    </div>
    ...
    <div class="bg-green-50 border-l-4 border-green-500 text-green-900 p-6 rounded-lg shadow mb-6 text-center">
      ...
      <p class="mt-2">Your Nhost project passes all checks.</p>
    </div>
    `}
    ...
    <div class="bg-white p-5 rounded-lg shadow mb-6">
      ...
      <p class="text-sm text-gray-600 mb-3">Copy and run in Nhost Dashboard → SQL Editor. Review each statement before executing.</p>
    ...
    <div class="text-center text-xs text-gray-500 mt-8 pb-4">
      Generated by <a href="https://github.com/Perufitlife/nhost-security-skill" class="text-emerald-700 underline">nhost-security</a>
